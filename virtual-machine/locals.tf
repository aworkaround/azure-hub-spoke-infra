...
locals {
  vm_name                      = var.vm_name != null ? lower(var.vm_name) : join("-", [lower(var.location), lower(var.label), "vm"])
  vm_size                      = var.vm_size
  ...
  resource_group_name          = var.resource_group_name != null ? var.resource_group_name : join("-", [lower(local.location), lower(var.label), "rg"])
  resource_group_count         = var.create_resource_group == true ? 1 : 0
  resource_group_data_count    = var.create_resource_group == true ? 0 : 1
  public_ip_name               = join("-", [lower(local.vm_name), "pip"])
  ...
  vnet_name                    = join("-", [lower(local.location), lower(var.label), "vnet"])
  subnet_name                  = join("-", [lower(local.location), lower(var.label), "snet"])
  ...
  network_interface_name       = join("-", [lower(local.vm_name), "nic"])
}
